<img src="bg.jpg" alt="banner" width="100%">

<h1 align="center">Hi 👋, I'm Rishikesh Patil</h1>

<div>
    <p>Social Handles:</p>
    <div style="width: 90%; display: flex; justify-content: space-between; margin-top: 10px;">
        <a href="https://www.linkedin.com/in/patilrishikesh" style="margin-right: 10px;">
            <img src="linkedin.png" alt="linkedin" width="30px">
        </a>
        <a href="https://wa.me/919137108042" style="margin-right: 10px;">
            <img src="whatsapp.png" alt="whatsapp" width="30px">
        </a>
        <a href="https://github.com/TheRishiPatil" style="margin-right: 10px;">
            <img class="social" src="github.png" alt="github" width="30px">
        </a>
        <a href="https://twitter.com/Rishi9137108042" style="margin-right: 10px;">
            <img src="twitter.png" alt="twitter" width="30px">
        </a>
        <a href="https://www.instagram.com/i_a_m_i_r_o_n_m_a_n/">
            <img src="instagram.png" alt="instagram" width="30px">
        </a>
    </div>
</div>
